(function () {
  ...
  function renderFacilityCards(facilities) {
    ...
    container.dataset.count = String(facilities.length);
    container.innerHTML = facilities
      .map((facility) => {
        ...
        const credentialMarkup = facility.credentials.length
          ? `<div class="facility-card-credentials" aria-label="Credential highlight">${facility.credentials.slice(0, 1).map((credential) => `<span>${escapeHtml(credential)}</span>`).join("")}</div>`
          : "";
        return `<article class="facility-card ${brandClass}">
          <button class="facility-card-select" type="button" data-facility-id="${escapeHtml(facility.id)}" aria-pressed="false" aria-label="Show ${escapeHtml(facility.brand)} ${escapeHtml(facility.name)} details on the map">
            <span class="facility-brand">${escapeHtml(facility.brand)}</span>
            <span class="facility-card-name">${escapeHtml(facility.name)}</span>
            <span class="facility-card-focus">${facility.capabilities.slice(0, 2).map(escapeHtml).join(" · ")}</span>
            ${credentialMarkup}
          </button>
        </article>`;
      })
  ...
  function selectFacilityCard(facilityId) {
    document.querySelectorAll(".facility-card-select").forEach((button) => {
      button.setAttribute("aria-pressed", String(button.dataset.facilityId === facilityId));
    });
  }

  function bindFacilityCardSelection() {
    document.querySelectorAll(".facility-card-select").forEach((button) => {
      button.addEventListener("click", () => {
        const marker = facilityMarkers.find((candidate) => candidate.options.facilityId === button.dataset.facilityId);
        if (!marker || !facilityMap) return;
        selectFacilityCard(button.dataset.facilityId);
        facilityMap.panTo(marker.getLatLng(), { animate: false });
        marker.openPopup();
      });
    });
  }
  ...
  function setFacilityFilter(filter) {
    ...
    facilityMarkers = visible.map((facility) => {
      ...
      marker.on("popupopen", () => selectFacilityCard(facility.id));
      marker.on("popupclose", () => selectFacilityCard(""));
      marker.addTo(facilityLayer);
      return marker;
    });
    bindFacilityCardSelection();
  ...
  function renderSchedule(schedule) {
    ...
    const sourceAgeDays = schedule.published_date ? Math.floor((Date.now() - new Date(schedule.published_date + 'T00:00:00').getTime()) / 86400000) : null;
    ...
    `;
    if (sourceAgeDays !== null && sourceAgeDays > 30) note.textContent += ' Historical snapshot (' + sourceAgeDays + ' days old); confirm availability with the mill before use.';
